...
def upsert_weather_station(weathercode, timestamp):
    ...
    if res.status_code in [204, 201]:
        print(f"[{timestamp}] [OK] Atualizado: {ENTITY_ID} com weathercode {weathercode}")
    else:
...
def create_subscription():
    ...
    res = requests.post(url, headers=HEADERS, json=subscription)
    if res.status_code in [200, 201]:
        print("[OK] Subscription criada com sucesso.")
    elif res.status_code == 422:
        print("[AVISO] Subscription já existe ou inválida.")
    else:
        print(f"[ERRO] Criando subscription: {res.status_code}")
        print(res.text)
